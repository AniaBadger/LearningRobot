...
//////////////////////////////// MUX SHIELD /////////////////////////////////////////
// Author: Anna Borsuk
// Based on example code at http://mayhewlabs.com/code/Mux_Shield_DigitalIn_Example.pde
...
// Method for reading value from chosen Mux shield pin (i) on any Multiplexer.
int readFromPin(int i, int mux) {
  int m;
  ...
  digitalWrite(m, HIGH); // target mux
...
// Method for writing HIGH value to chosen Mux shield pin (i) on Multiplexer 0.
void writeHighToPin(int i) {
  pinMode(14, OUTPUT); // mux 0
  pinMode(15, INPUT);
  pinMode(16, INPUT);
...
// Method for writing LOW value to chosen Mux shield pin (i) on Multiplexer 0.
void writeLowToPin(int i) {
  pinMode(14, OUTPUT); // mux 0
  pinMode(15, INPUT);
  pinMode(16, INPUT);
  digitalWrite(CONTROL0, (i&15)>>3); //S3
...
// Added special method by Anna Borsuk for PulseIn that works with Mux Shield.
...
int echoPin = 0;                                    // SRF05 echo pin (Mux0 pin 0)
int initPin = 1;                                    // SRF05 trigger pin (Mux0 pin 1)
unsigned long pulseTime = 0;                    // stores the pulse in Micro Seconds
unsigned long distance = 0;                     // variable for storing the distance (cm)

// Perform PulseIn command on Mux0 pin.
unsigned long muxPulseIn(int i) {
  pinMode(14, INPUT); // mux 0
  digitalWrite(CONTROL0, (i&15)>>3); //S3
  digitalWrite(CONTROL1, (i&7)>>2);  //S2
  digitalWrite(CONTROL2, (i&3)>>1);  //S1
  digitalWrite(CONTROL3, (i&1));     //S0
  return pulseIn(14, HIGH);
}

////////////////////////////////// FLEX SEMSORS + IR ///////////////////////////////////
// Author: Anna Borsuk

int mux1arrayA[5], mux1arrayD[5];
int frontLeft, frontRight, backLeft, backRight, ir;

////////////////////////////////// MOTOR DRIVER ///////////////////////////////////
// Author: Anna Borsuk
// Based on sample code from http://www.seeedstudio.com/depot/datasheet/L298%20Dual%20H-Bridge%20Motor%20Driver%20datasheet.pdf
...
void setup() {
  ...
  pinMode(EA,OUTPUT); 
  digitalWrite(EA,HIGH); // set EA port to digital high for it to run
  ...
  pinMode(EB,OUTPUT); 
  digitalWrite(EB,HIGH); // set EB port to digital high for it to run
...
void loop() {
  ...
  for (int i=0; i<5; i++)
  {
    ...
    digitalWrite(CONTROL2, (i&3)>>1);  
    digitalWrite(CONTROL3, (i&1));     
    mux1arrayA[i] = analogRead(1);
    mux1arrayD[i] = digitalRead(15);
  }
  
  frontLeft = mux1arrayA[1]; // Analog data from pin 1 on Mux1
  frontRight = mux1arrayA[3]; // Analog data from pin 3 on Mux1
  backLeft = mux1arrayA[0]; // Analog data from pin 0 on Mux1
  backRight = mux1arrayA[2]; // Analog data from pin 2 on Mux1
  
  ir = mux1arrayD[4]; // Digital data from pin 4 on Mux1
  
  //////////////////////////////// ULTRASONIC SENSOR /////////////////////////
  // written at: luckylarry.co.uk
  
  writeHighToPin(initPin);                // send 10 microsecond pulse
  delayMicroseconds(10);                  // wait 10 microseconds before turning off
  writeLowToPin(initPin);                 // stop sending the pulse
  
  // Use custom PulseIn method
  pulseTime = muxPulseIn(echoPin);        // Look for a return pulse, it should be high as the pulse goes low-high-low
  ...
  if (Serial.available() >0) {
    ...
    // reset counter when connection established.
    count = 0;
    ...
    switch (s) {
	  ...
	  /* the first received value indicates the mode       
           ...
           s=10 is Move Forward;  
           s=20 is Move Backward;  
           s=30 is Move Left;  
           s=40 is Move Right; 
           s=50 is Stop; 
           s=60 is read IR;  
           s=70 is read Front Left Flex;
           s=80 is read Front Right Flex;
           
           s=90 is query script type (1 basic, 2 motor);
           
           s=100 is read Back Right Flex;
           s=210 is read Ultrasonic Sensor;
           s=220 is read Back Left Flex;
      ...
      if ((s>100 && s<210) || (s>250 && s!=340 && s!=400)) {
        s=-1;
      }
      ...
      ////////////////////////////////// CUSTOM ROBOT COMMANDS START HERE ///////////////////////////////////
      // Author: Anna Borsuk, basing on existing case examples.
      
      ///// s=10 means MOVE FORWARD /////      
      case 10:
      /* the second received value indicates the pin 
         from abs('c')=99, pin 2, to abs('¦')=166, pin 69    */
      if (val>98 && val<167) {
        ...
        Serial.println(1, DEC);       /* send acknowledgement via serial  */
      }
	...
	digitalWrite(pin,dgv);     /* perform Digital Output */
      ...
      if (val>98 && val<167) {
        ...
        Serial.println(ir, DEC);       /* send sensor reading via serial  */
      ...
      if (val>98 && val<167) {
        ...
        Serial.println(frontLeft, DEC);       /* send sensor reading via serial  */
      ...
      if (val>98 && val<167) {
        ...
        Serial.println(frontRight, DEC);       /* send sensor reading via serial  */
      ...
      ///// s=90 means SACRED DO NOT TOUCH!!!!! Query Script Type (1 basic, 2 motor) /////
      ...
      if (val>98 && val<167) {
        ...
        Serial.println(backRight, DEC);       /* send sensor reading via serial  */
        
      }
      s=-1;  /* we are done with US so next state is -1      */
      break; /* s=100 taken care of                           */



      ///// s=210 means read Ultrasonic Sensor /////
      
      case 210:
      if (val>98 && val<167) {
        
        Serial.println(averageDistance, DEC);       /* send sensor reading via serial  */
      }
      ...
      if (val>98 && val<167) {
        ...
        Serial.println(backLeft, DEC);       /* send sensor reading via serial  */
      }
      ...
      ////////////////////////////////// CUSTOM ROBOT COMMANDS END HERE ///////////////////////////////////
  ...
  // If serial not available (connection lost) for 500 arduino loops (less then half a second) then stop the robot.
