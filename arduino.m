classdef arduino < handle
    ...
    methods
        ...
        function a=arduino(comPort)
            ...
            a.aser=serial(comPort,'BaudRate',4800);
        ...
        %%%% Functions originally written by Anna Borsuk start here %%%%%%%
        
        %%% Move robot depending on direction given (d) and time specified
        %%% (t - Matlab loops). Nested while loop makes sure if the wifi
        %%% connection is lost, the robot will continue with the planned
        %%% movement when it is back again. Numer 2 in brackets doesn't
        %%% mean anything - in the original code it meant the pin number we
        %%% want to target, but that is not neccessary now.
        function moveRobot(a,d,t)
            if strcmp(d,'f')
                while t > 0
                    move = a.moveForward(2);
                    while isempty(move)
                        move = a.moveForward(2);
                    end
                    t = t - 1;
        ...
        %%% Read ultrasonic sensor reading. Flag the presense of an
        %%% obstacle when the distance is less than 7cm. Nested while loop 
        %%% makes sure if the wifi
        %%% connection is lost, the robot will continue to try and read the
        %%% sensor readings until it's back again.
        function val = us(a)
            us = a.ultrasonicSensor(2);
            while isempty(us)
                us = a.ultrasonicSensor(2);
            end
            if (us < 7)
        ...
        %%% Read front flex sensors readings. Flag the front collision with an
        %%% obstacle when the bend degree of left flex is more than 500 or the bend degree of
        %%% right flex is more than 570. Nested while loop 
        %%% makes sure if the wifi
        %%% connection is lost, the robot will continue to try and read the
        %%% sensor readings until it's back again.
        function val = frontFlex(a)
            fl = a.frontLeft(2);
            fr = a.frontRight(2);
            while isempty(fl) || isempty(fr)
                fl = a.frontLeft(2);
                fr = a.frontRight(2);
            end
            if (fl > 500) || (fr > 570)
        ...
        %%% Get robot's state fron States matrix (S) based on Sensor readings (Sensors) and action taken.
        function [sensorState,state] = getState(a,Sensors,action,S)
            us = a.us();
            ff = a.frontFlex();
            SensorReading = [us ff];
            [~,indx] = ismember(SensorReading,Sensors,'rows');
            sensorState = indx;
            state = S(sensorState,action);
        end
        
        %%% Get maximum Q action value of state from Q matrix.
        function val = maxQAction(~,Q,state,numberOfActions)
            mqa = -inf;
            for i=1:numberOfActions
                mqa = max([mqa Q(state,i)]);
            end
            val = mqa;
        end
        
        %%%% Functions originally written by Anna Borsuk end here %%%%%%%
        
        %%%% Functions based on pre-existed ArduinoIO library ones modified by Anna Borsuk start here %%%%%%%
        
        % moveForward
        function val=moveForward(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[49 97+pin],'uchar'); % NUMBER 49 gives case 10
                
                % Get action acknowledgement.
                val=fscanf(a.aser,'%d');
        ...
        % moveBackward
        function val=moveBackward(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[50 97+pin],'uchar'); % NUMBER 50 gives case 20
        ...
        function val=moveLeft(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[51 97+pin],'uchar'); % NUMBER 51 gives case 30
        ...
        function val=moveRight(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[52 97+pin],'uchar'); % NUMBER 52 gives case 40
        ...
        function val=stop(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[53 97+pin],'uchar'); % NUMBER 53 gives case 50
        ...
        function val=IRsensor(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[54 97+pin],'uchar'); % NUMBER 54 gives case 60
        ...
        function val=frontLeft(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[55 97+pin],'uchar'); % NUMBER 55 gives case 70
        ...
        function val=frontRight(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[56 97+pin],'uchar'); % NUMBER 56 gives case 80
        ...
        function val=backLeft(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[70 97+pin],'uchar'); % NUMBER 70 gives case 220
        ...
        function val=backRight(a,pin)            
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[58 97+pin],'uchar'); % NUMBER 58 gives case 100
        ...
        % ultrasonicSensor
        function val=ultrasonicSensor(a,pin)                    
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % Send command number and pin (pin not really needed, but
                % did not want to change it as long as the code was working).
                fwrite(a.aser,[69 97+pin],'uchar'); % NUMBER 69 gives case 210
        ...
        %%%% Functions based on pre-existed ArduinoIO library ones modified by Anna Borsuk end here %%%%%%%
